...
function Artifact() {
  ...
  React.useEffect(() => {
    ...
    const handlePointerMove = (event) => {
      setArtifactWidth(Math.min(720, Math.max(300, window.innerWidth - event.clientX)))
    }
    ...
    <>
      ...
      <AnimatePresence>
        {mobileOpen && <>
          ...
          <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} transition={{ duration: 0.25, ease: "easeInOut" }}           className="lg:hidden fixed inset-y-0 right-0 z-50 w-[min(88vw,420px)] max-w-full border-l border-white/[0.06] overflow-hidden">
      ...
      <motion.div
        initial={{ width: 400 }}
        animate={{ width: collapsed ? 48 : artifactWidth }}
        transition={{
        ...
        {!collapsed && (
          <div
            ...
            className='absolute left-[-4px] top-0 z-30 h-full w-2 cursor-col-resize touch-none hover:bg-indigo-500/60 transition-colors'
